...
exports.userSignup = async (req, res) => {
    try {
        ...
        return proccedToSignUp(reqBodyData, res)
    } catch (error) {
        console.log(error)
        res.send({
            status: {
...
const proccedToSignUp = async (reqBodyData, res) => {
    try {
...
const sendEmail = (data, res) => {
    ...
    ejs.renderFile(templatePath, templateVariables, (err, htmlContent) => {
        ...
        const mailOptions = {
            from: 'abhishek0409shinde@gmail.com',  // Sender email address
...
exports.pendingVisit = async (req, res) => {
    try {
        ...
        return res.render('dashboard', {
            isError: true,
            message: "Visits details has been moved to pendig.",
            visitsData: pendingVisits,
            admin:req.session.admin
